...
- **Backup & Restore (merge mode)**: `BackupService.applyBackupData(...)` is now a **non-destructive merge**. WebApps are matched by normalized URL (`normalizeAppKey`) and never duplicated; existing apps are kept untouched and the backup-side id is remapped to the local id via `appIdMap`. Per-app data is deduped at insert time: clipboard `(appId,content,timestamp)`, credentials `(appId,title,username)`, downloads `(appId,url,fileName)`, presets `name`, notifications `(appName,title,message,timestamp)`. Single-row settings (notification/security) only restore if no row currently exists. `mergeSharedPreferences` only sets keys that aren't already present locally. The restore result message reports counts of newly added vs. matched items.
- **Floating Windows**: Apps can be floated as chat-head overlays (permission check + FloatingWindowService). Modern UI: rounded glass window with header (app-icon badge + title + URL chip + traffic-light style minimize/maximize/close + overflow). Overflow opens a custom **glass action panel** (`floating_action_panel.xml` + `floating_action_tile.xml`) added as its own overlay window — sectioned into Navigation / Modes (toggle pills) / Page / Tools. The legacy `PopupMenu` dropdown was replaced by this panel. The action panel content lives inside a `ScrollView` (`actionPanelScroll`) whose height is capped at ~50% of the screen at show-time so it never overflows. The Tools row (Vault / Clipboard / Downloads) opens **inside the floating window itself** via `floating_tool_panel.xml` (sub-header with back arrow + title + count, scrollable list of `floating_tool_row.xml` items) — `showToolInWindow(...)` hides the WebView+resize-handle and inflates the tool panel into `R.id.webViewContainer`; `dismissToolPanel(...)` (also called on minimize/close) restores the WebView. Items are loaded one-shot via `Flow.firstOrNull()` from the same DAOs the full-screen activities use (`waosClipboard.getAppClipboardItems`, `credentialManager.getDecryptedCredentialsForApp`, `downloadManager.getDownloadsForApp`). The URL chip itself contains a pencil icon and is clickable; tapping it opens a glass **URL editor** overlay (`floating_url_editor.xml`) with an inline EditText (textUri + IME action Go), clear button, Cancel/Go buttons, and dim-behind. On submit, raw input is normalized via `normalizeUrl(...)` (auto-adds `https://`, falls back to a Google search if the input has no dot/contains spaces), the WebView navigates, and the visit is recorded to the per-app link history via `LinkHistoryTracker(appContext, appId).recordAction(action = "manual_navigate", referrer = previousUrl)`. All styling lives in `fw_*` drawables and `fw_*` colors; tile/section styles in `floating_window_styles.xml`.
- **Download Manager**: Per-app download history (DownloadHistoryActivity with EXTRA_DOWNLOAD_APP_ID). All downloads, blob saves, data-URL saves, screenshots and session exports are written to **app-private external storage** under `context.getExternalFilesDir(null)/WAOS/{AppName}/{FileType}/` instead of the public `/Download/WAOS/...` folder. Files are NOT media-scanned (`MediaScannerConnection` calls were removed), so they never appear in Gallery / device file manager. The app retains full read/write access without storage permissions, FileProvider already exposes `external-files-path` so opening/sharing through `${pkg}.fileprovider` continues to work, and `DownloadItem.filePath` keeps absolute paths so existing history entries that point at the old public folder remain readable.
...
13. **Copies from WebView popup not appearing in Clipboard Manager**: Three user-facing copy paths only set the system clipboard but never persisted into the per-app Clipboard Manager database — so copying a link from the long-press popup, copying selected text from the selection bar, or hitting the toolbar Copy URL button silently bypassed the in-app history. Fixed by routing all three through `viewModel.captureClipboardItem(text, hint)` (which de-dupes within 1.5s, auto-detects URL type, and writes to the `clipboard_items` table via `ClipboardManager.copyToAppClipboard`), and updated each toast to reflect that the item was saved (`"Link copied — saved to Clipboard Manager"`, `"URL copied — saved to Clipboard Manager"`). Also fixed the **floating window** Copy URL action (`FloatingWindowService.ACTION_TOOLBAR_COPY_URL`) which was the same bypass — injected the WAOS `ClipboardManager` (`com.cylonid.nativealpha.manager.ClipboardManager`) into the service via Hilt and call `waosClipboard.copyToAppClipboard(appId, url, ClipboardItem.Type.URL)` so floating-window URL copies also appear in the per-app history with a confirmation toast.
